  ...
  return linter((view) => {
    ...
    for (let i = 0; i < content.length; i++) {
      ...
      if (Object.keys(brackets).includes(char)) {
        ...
        const expectedOpeningEntries = Object.entries(brackets).filter(
          ([, v]) => v === char,
        );
        const expectedOpening =
          expectedOpeningEntries.length > 0 ? expectedOpeningEntries[0][0] : "";
...
export const createErrorTheme = () => {
  return EditorView.theme({
    ".cm-diagnosticUnderline": {
      backgroundImage:
        'url(\'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="6" height="3"><path fill="red" d="M0,2.5 C1,1.5 2,0.5 3,1.5 S5,3.5 6,2.5"/></svg>\')',
