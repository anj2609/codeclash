...
const TestCases = forwardRef<TestCasesRef, TestCasesProps>(({ 
  ...
  useEffect(() => {
    if (runResult !== null && runResult !== undefined && !isRunning && !runError && !isCustomInputMode) {
  ...
  }, [runResult, runError, isRunning, activeCase, testCases, isCustomInputMode]);
    ...
    <div className="h-full flex flex-col bg-[#1C202A] rounded-lg">
      <div className="flex items-center p-4 border-b border-[#1E2D3D]">
        ...
        <div className="flex ml-auto">
          <button
            ...
            title={isCustomInputMode ? "Switch to test cases" : "Switch to custom input"}
          >
            <Pencil size={16} />
          </button>
        </div>
      ...
      <div className="flex-1 grid grid-rows-2 p-4 gap-4 overflow-hidden">
        <div className="flex flex-col min-h-0">
          <h4 className="text-sm text-gray-400 mb-1">{isCustomInputMode ? "Custom Input:" : "Input:"}</h4>
          ...
          {(runResult !== null && runResult !== undefined || runError) && (
            <div className={isCustomInputMode ? "" : "mt-2"} ref={resultsRef}>
              <div className="flex justify-between items-center mb-1">
                ...
                {!isRunning && !runError && isCorrect !== null && !isCustomInputMode && (
              ...
              {isRunning ? (
                ...
                <div className={`rounded-md p-3 text-sm overflow-auto ${
                  isCorrect === true && !isCustomInputMode ? 'bg-green-900/20 border border-green-600/30 text-green-200' :
                  isCorrect === false && !isCustomInputMode ? 'bg-red-900/20 border border-red-600/30 text-red-200' :
                  'bg-[#292C33] text-white'
                }`}>
                  {runResult === '' ? (
